"""TypeSafe client helpers. Lifecycle owned by companion/analyze commands."""
...
def ensure_typesafe_sdk() -> None:
    ...
    try:
        ...
        from ghostcopyeditor.paths import package_project_root
        ...
        raise TypesafeConfigError(
            ...
            "  Or pass --no-typesafe to use deterministic checkers only."
        ) from exc
...
def ensure_typesafe_api_key() -> None:
    ...
    if not key:
        raise TypesafeConfigError(
            ...
            "  Add it to secrets/llm.env, or pass --no-typesafe to use "
            "deterministic checkers only."
        )
